...
DATE="2026-07-11"
HDR='"Developer","Date","Hours","Files Count"'
...
# Test 2: Single commit — 0.00h, 1 file
echo "=== Single commit ==="
d=$(mktemp -d); (cd "$d" && git init -q && git config user.email "t@t.com" && git config user.name "Test Dev" && echo "x" > f && git add f && git commit -q -m "c1" && $SCRIPT -o "$d/out.csv" >/dev/null 2>&1)
ROW='"Test Dev","'"$DATE"'","0.00","1"'
