---
description: Audit any codebase as a senior advisor and write self-contained spec
  prompts under specs/<spec>/improve/ for the spec-kit lifecycle to process. Discovery
  mode surfaces prioritized findings with evidence; a specific change writes a single
  prompt for just that; re-running refreshes prompts whose code has drifted
---


<!-- Extension: improve -->
<!-- Config: .specify/extensions/improve/ -->
# Improve
...
- `--issues`: also publish each written prompt as a GitHub issue. Read the "Issues" section of `.specify/templates/improve-closing-the-loop.md` first. Only with the explicit flag.
...
- Audit playbook: `.specify/templates/improve-audit-playbook.md` (what to look for per category, the finding format, the prioritization rubric).
- Spec prompt template: `.specify/templates/improve-spec-prompt-template.md` (the structure and placement rules every written prompt follows).
- Follow-through reference: `.specify/templates/improve-closing-the-loop.md` (the handoff to `/speckit.specify` and the `--issues` flow; read its Handoff section for the next steps to report, and its Issues section before `--issues`).
...
Audit the codebase across the categories in `.specify/templates/improve-audit-playbook.md`. Read it now. Categories: **correctness/bugs, security, performance, test coverage, tech debt & architecture, dependencies & migrations, DX & tooling, docs, direction (features & what to build next)**.
...
For each selected finding, write one spec prompt file using `.specify/templates/improve-spec-prompt-template.md`. Read it before writing the first prompt. Prompts go in:
